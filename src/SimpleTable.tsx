 ...
 * @param input 
 */
function objectToTable1(input?: { [key: string]: string }[] | null): json2md.DataObject | undefined {
  let jsonObj: json2md.DataObject | undefined;
  if (Array.isArray(input) === true && input?.length !== 0) {
    jsonObj = {
      table: {
        headers: input !== null && input !== undefined ? Object.keys(input[0]) : [],
        rows: input ?? [],
      },
    };
  }
...
function downloadAsOnject(data: any, exportName: any): void {
  let dataStr =
    "data:text;charset=utf-8," + encodeURIComponent(json2md(jsonObj));
...
function jspPdfGenerator(): void {
  ...
  doc.text(100, 100, "This is rahul jha");

  doc.textWithLink("Vist StackOverflow here", 100, 120, {
  ...
  doc.addImage(imageAddr, "JPEG", 140, 140, 50, 50);
  // doc.text(20,20,myObj1);
...
export default function SimpleTable() {
  ...
  return (
    <div>
      ...
      <button onClick={() => downloadAsOnject(ObjectToTable1(data), "rahul")}>
        GenerateMD
      </button>
    </div>
  );
